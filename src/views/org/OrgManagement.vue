...
const tabs = [
    ...
    { key: 'requests', label: 'Requests & Invites', icon: MailOpen },
...
<template>
    <div class="min-h-screen bg-gray-100 dark:bg-abyss-900">
        <header class="border-b border-gray-200 dark:border-abyss-700 bg-white dark:bg-abyss-800 sticky top-0 z-10 shadow-lg">
            <div class="max-w-7xl mx-auto px-6 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-5">
                        <div class="w-12 h-12 bg-gray-200 dark:bg-abyss-700 rounded-lg flex items-center justify-center p-1 shadow-inner ring-1 ring-gray-300 dark:ring-platinum-700">
                            <img v-if="organization?.logo" :src="getLogoUrl(organization.logo)" :alt="`${organization?.name} logo`"
                                class="w-full h-full rounded-lg object-cover" />
                            <Image v-else class="w-7 h-7 text-gray-500 dark:text-platinum-500" />
                        </div>
                        
                        <div>
                            <h1 class="text-2xl font-bold text-gray-800 dark:text-platinum-50">{{ organization?.name }}</h1>
        ...
        <div class="border-b border-gray-200 dark:border-abyss-700 bg-gray-50 dark:bg-abyss-800 shadow-inner">
        ...
        <div class="max-w-7xl mx-auto px-6 py-8">
            <div v-if="loading" class="flex flex-col items-center justify-center py-20 bg-white dark:bg-abyss-800 rounded-xl border border-gray-200 dark:border-abyss-700 shadow-xl">
