...
<template>
    <div>
        <div class="flex items-center justify-between mb-4">
            ...
            <h2 class="text-xl font-semibold text-abyss-900 dark:text-platinum-50">
                My Join Requests
            </h2>
        ...
        <div v-else-if="!filteredRequests.length"
            ...
            <p class="text-abyss-700 dark:text-platinum-400">
            ...
            <div v-for="request in filteredRequests" :key="request.id"
                ...
                <div class="flex items-start justify-between mb-3">
                    <div class="flex-1">
                        ...
                        <RouterLink :to="{
                            name: 'orgs.org',
                            params: { id: request.organization_id },
                        }" class="text-lg font-semibold text-kaitoke-green-600 dark:text-kaitoke-green-300 hover:underline mb-1 block">
                            {{ request.organization_name }}
                        </RouterLink>

                        <div class="flex items-center gap-1 text-sm text-abyss-700 dark:text-platinum-400">
                            <Clock class="w-4 h-4" :stroke-width="1"/>
                ...
                <div v-if="request.message"
                    ...
                    <p class="text-abyss-800 dark:text-platinum-300 text-sm">
                        {{ request.message }}
                    </p>
                </div>

                <!-- Updated admin note styling for light and dark mode -->
                <div v-if="request.admin_note && request.status !== 'pending'" class="mb-3 p-3 rounded border" :class="request.status === 'approved'
                        ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-700/50'
                        : 'bg-rose-50 dark:bg-rose-900/20 border-rose-200 dark:border-rose-700/50'
                    ">
                    <p class="text-xs text-abyss-700 dark:text-platinum-500 mb-1">
                        Admin Response:
                    </p>
                    <p class="text-abyss-800 dark:text-platinum-300 text-sm">
                        {{ request.admin_note }}
