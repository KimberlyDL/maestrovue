<template>
    <div class="space-y-8">
        ...
        <div v-if="activeTab === 'requests'">
            <div class="bg-white dark:bg-abyss-800 border border-gray-200 dark:border-abyss-700 rounded-xl p-5 shadow-lg mb-6">
            ...
            <div class="space-y-4">
                <div v-for="request in filteredRequests" :key="request.id"
                    class="bg-white dark:bg-abyss-800 border border-gray-200 dark:border-abyss-700 rounded-xl p-6 shadow-md hover:border-kaitoke-green-600/30 dark:hover:border-abyss-600 transition-all duration-200">
                    ...
                    <div v-if="request.message" class="mb-4 p-3 bg-gray-50 dark:bg-abyss-900 rounded-lg border border-gray-200 dark:border-abyss-600 shadow-inner">
                    ...
                    <div v-if="request.status === 'pending'" class="flex gap-4 pt-2">
                        <button @click="approveRequest(request)" :disabled="isProcessing"
                            class="flex-1 py-2.5 px-4 bg-kaitoke-green-600 hover:bg-kaitoke-green-500 disabled:opacity-50 text-white font-medium rounded-xl transition-colors shadow-md flex items-center justify-center gap-2">
                            <Check class="w-4 h-4" /> Approve
                        </button>
                        <button @click="declineRequest(request)" :disabled="isProcessing"
                            class="flex-1 py-2.5 px-4 bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white font-medium rounded-xl transition-colors shadow-md flex items-center justify-center gap-2">
                            <X class="w-4 h-4" /> Decline
                        </button>
                    </div>
                </div>

                <div v-if="filteredRequests.length === 0" class="text-center py-12 bg-white dark:bg-abyss-800 rounded-xl border border-gray-200 dark:border-abyss-700 shadow-md">
        ...
        <div v-else>
            <div class="bg-white dark:bg-abyss-800 border border-gray-200 dark:border-abyss-700 rounded-xl p-6 shadow-xl">
                ...
                <div v-if="inviteCode" class="space-y-4">
                    <div>
                        ...
                        <div class="flex gap-3">
                            ...
                            <button @click="copyInviteCode"
                                class="px-5 py-2.5 rounded-xl border border-gray-300 dark:border-platinum-700 text-gray-700 dark:text-platinum-200 hover:bg-gray-100 dark:hover:bg-abyss-700 transition-colors shadow-sm flex items-center gap-2 font-medium">
                    ...
                    <div class="flex gap-3 pt-3 border-t border-gray-200 dark:border-abyss-700">
                        <button @click="generateNewCode" :disabled="isGenerating"
                            class="px-5 py-2.5 rounded-xl bg-kaitoke-green-600 hover:bg-kaitoke-green-500 disabled:opacity-50 text-white font-medium shadow-md flex items-center gap-2">
                            <RotateCcw class="w-4 h-4" /> {{ isGenerating ? 'Generating...' : 'Generate New Code' }}
                        </button>
                        <button @click="confirmRemoveCode" :disabled="isRemoving"
                            class="px-5 py-2.5 rounded-xl border border-rose-400/50 text-rose-600 dark:text-rose-300 hover:bg-rose-100 dark:hover:bg-rose-900/30 disabled:opacity-50 font-medium transition-colors shadow-sm flex items-center gap-2">
                ...
                <div v-else class="text-center py-10">
                    ...
                    <button @click="generateNewCode" :disabled="isGenerating"
                        class="px-6 py-3 rounded-xl bg-kaitoke-green-600 hover:bg-kaitoke-green-500 disabled:opacity-50 text-white font-semibold shadow-lg flex items-center gap-2 mx-auto">
                        <Key class="w-5 h-5" /> {{ isGenerating ? 'Generating...' : 'Generate Invite Code' }}
                    </button>
                </div>

                <div class="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-300 dark:border-blue-700/40 rounded-lg shadow-sm">
